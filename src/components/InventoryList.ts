...
export default class InventoryList extends BaseComponent {
  ...
  private itemPriceLocator = this.root.locator(
    '[data-test="inventory-item-price"]',
  );
  private itemAddToCartButtonLocator = this.root.getByText("Add to cart", {
    exact: true,
  });
  ...
  async parseAllItems() {
    ...
    for (let i = 0; i < itemsCount; i++) {
      const item: Item = {
        ...
        removeButtonDataTest: (
          await this.getItemAddToCartButtonDataTest(i)
        ).replace("add-to-cart", "remove"),
      };
  ...
  private async getItemDescription(index: number): Promise<string> {
    const description = await this.itemDescriptionLocator
      .nth(index)
      .textContent();
    if (description === null) {
  ...
  private async getItemAddToCartButtonDataTest(index: number): Promise<string> {
    const price = await this.itemAddToCartButtonLocator
      .nth(index)
